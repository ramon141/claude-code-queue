...
class RateLimitInfo:
    ...
    def from_claude_response(cls, response_text: str) -> 'RateLimitInfo':
        ...
        is_limited = any(indicator in response_text.lower() for indicator in rate_limit_indicators)
...
class QueueState:
    ...
    def get_next_prompt(self) -> Optional[QueuedPrompt]:
        ...
        if not executable_prompts:
            ...
            retry_prompts = [
                p for p in self.prompts
                if p.status == PromptStatus.RATE_LIMITED and p.should_execute_now() and p.can_retry()
            ]
            if retry_prompts:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for status in PromptStatus:
            status_counts[status.value] = len([p for p in self.prompts if p.status == status])
        ...
        return {
            ...
            "current_rate_limit": {
                "is_rate_limited": self.current_rate_limit.is_rate_limited if self.current_rate_limit else False,
                "reset_time": self.current_rate_limit.reset_time.isoformat() if self.current_rate_limit and self.current_rate_limit.reset_time else None
            }
        }
